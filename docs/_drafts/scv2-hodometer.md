...
Core v2 has multiple Kubernetes custom resources --- `Models`, `Servers`, `Pipelines`, and `Experiments` --- compared to just a single one in Core v1 --- `SeldonDeployments`.
...
As it happens, they are both implemented by the `kubeClientWrapper` struct in [kubernetes.go](https://github.com/kubernetes-retired/spartakus/blob/a593c5e0f13efa415752bf4ebaccdacddf7e979a/pkg/volunteer/kubernetes.go#L138), but this is an implementation detail.
